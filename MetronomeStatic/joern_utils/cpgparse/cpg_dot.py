    ...
    def __init__(self, graph) -> None:
        ...
        for node_id in self.graph.nodes:
            ...
            if label.lstrip("(").startswith(
                ...
                expr = "DDGNodeRoot"(label.lstrip("(").split(",")[0].strip(), None)
...
def extract_line_from_label(label: str) -> int:
    res = re.search(r"<SUB>(?P<line>\d+)</SUB>", label)
    if res is None:
        return -1
    else:
        return int(res.groupdict()["line"])


def pydot_to_nx(dot_graph: pydot.Dot):
    G = nx.DiGraph()
    edges = (
    ...
    nodes = (
        ...
        .map(
            lambda node: (
                node.get_name(),
                node.obj_dict["attributes"].get("label"),
                extract_line_from_label(node.obj_dict["attributes"].get("label", "")),
            )
        )
        .f
    )
    for node_id, label, line in nodes:
        G.add_node(node_id, label=label, line=line)
