...
class CFGBuilder:
    ...
    def visitBinaryOperator(self, B):
        ...
        if B.isAssignmentOp():
            self.autoCreateBlock()
            self.appendStmt(self._block, B)
            # self._block.put_statement(B._cursor)
    ...
    def visitCompoundAssignmentOp(self, C: CompoundAssignmentOp):
        ...
        self.autoCreateBlock()
        self.appendStmt(self._block, C)
        # self._block.put_statement(C._cursor)
    ...
    def visitSwitchStmt(self, terminator):
        ...
        self._block.set_predicate("switch", terminator.getCond()._cursor)
    ...
    def appendStmt(self, CFGBlock: CFGBlock, stmt):
...
class CFG:
    def __init__(self):
        self._entry = None
        self._exit = None
        self._blocks: CustomVector[CFGBlock] = CustomVector()
        self._numBlockID = -1
