...
from .decorators.concrete_node_decorators import GenericDecorator
...
PredicateKinds = Literal["if", "while", "switch"]
...
class CFGBlock:
    ...
    def __init__(self, blockID, C, CFGparent):
        ...
        self._elements: CustomVector[GenericDecorator] = C

        # TODO: label = stmt()
        self._label = None

        # The terminator for a basic block that indicates the type of control-flow
        # that accours between a block and its successors.
        ...
        # Predicate used in conditional blocks
        # such as if/while conditions
        self._predicate: Optional[Tuple[PredicateKinds, Cursor]] = None

        self._statements: List[Cursor] = []

    def set_predicate(self, kind: PredicateKinds, c: Cursor):
        self._predicate = (kind, c)

    def put_statement(self, statement):
        assert isinstance(statement, Cursor), statement
        self._statements.append(statement)

    def get_statements(self) -> Generator[Cursor, None, None]:
        for element in self._statements:
            yield element
