...
看到这里，你或许会想：直接从AST转换为CFG不好吗？比如第一张图中，三种语言的AST到CFG中间只有三个箭头，而第二张图中，却是有了4个箭头，看似反而是多了一步。**但实际上，AST-->UAST的变换，比AST-->CFG的变换简单的多**。虽然都是遍历抽象语法树，但前者只要递归遍历抽象语法树就好，后者除了遍历语法树，还需要仔细管理分析的状态，难度和测试用例的复杂程度不可同日而语。
