...
def test_slice():
    print("x", SkyGenerator(iter([1, 2, 3, 4, 5])).slice(1, 3).to_list())
    assert [2, 3] == SkyGenerator(iter([1, 2, 3, 4, 5])).slice(1, 3).to_list()
    assert [1, 2, 3] == SkyGenerator(iter([1, 2, 3, 4, 5])).slice(0, 3).to_list()

    g = SkyGenerator(range(5))
    assert [0, 1, 2] == g.slice(0, 3).to_list()
    g = SkyGenerator(range(5))
    assert [3, 4] == g.slice(3, -1).to_list()


def test_reduce():
    g = SkyGenerator(iter([1, 2, 3, 4, 5])).freeze()
    ret = g.reduce(lambda sum, b: sum + b, 0)
    assert ret == 15
    ret = g.slice(4).reduce(lambda sum, b: sum + b, 0)
    assert ret == 10


def test_iteration_on_frozen():
    g = SkyFrozenGenerator([1, 4, 3, 2, 5])
    for item in g:
        pass
...
def test_iteration_with_freeze():
    g = SkyGenerator([1, 2, 3, 4, 5]).freeze()
...
def test_star():
    assert [0, 2, 4, 6, 8] == SkyGenerator(((i, i) for i in range(5))).star_map(
        lambda x, y: x + y
    ).to_list()

    assert [(1, 1), (3, 3), (5, 5), (7, 7), (9, 9)] == SkyGenerator(
...
def test_indexed():
    ret = (
        SkyGenerator([1, 3, 4, 2, 1])
        .indexed_filter(lambda i, val: val % 2 == 0 and i % 2 == 0)
        .to_list()
    )
    assert ret == [4]

    assert [0, 4, 6, 6] == SkyGenerator([0, 4, 3, 2]).indexed_map(
        lambda i, val: i * val
    ).l
